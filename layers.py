...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        attn_out, self.attn = attention(q, k, v, mask=mask, dropout=self.dropout)
        # concat
        attn_out = attn_out.transpose(1, 2).contiguous().view(bs, -1, self.h*self.d_k)
        return self.linears[3](attn_out)
